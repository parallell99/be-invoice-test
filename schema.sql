-- Invoice app schema (PostgreSQL 16+) — snapshot for reference
-- แหล่งหลัก: BE/migrations/*.sql — รัน npm run migrate จากโฟลเดอร์ BE
-- Database name from docker-compose: invoice
-- Apply แบบครั้งเดียว (cwd = BE): psql -U postgres -d invoice -f schema.sql

BEGIN;

-- ---------------------------------------------------------------------------
-- users (ชื่อตารางไม่ใช้ "user" เพราะเป็นคำสงวนใน PostgreSQL)
-- ---------------------------------------------------------------------------
CREATE TABLE users (
...
-- ---------------------------------------------------------------------------
-- อัปเดต updated_at อัตโนมัติเมื่อแก้ invoice
-- ---------------------------------------------------------------------------
